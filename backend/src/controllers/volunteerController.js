...
exports.replyToQuestion = async (req, res) => {
  try {
    ...
    if (String(question.collegeCode) !== String(req.volunteer.collegeCode)) {
      return res.status(403).json({ success: false, message: "You can only reply to questions from your college" });
    }
